...
singularity exec -B ../tmp_group_split:/opt/dataset odm_latest.sif python3 /code/run.py --max-concurrency $SLURM_NTASKS --feature-quality medium --project-path "/opt/dataset" $uid_key
## try to remove the opensfm folder that takes up so much space
rm -rf ../tmp/$uid_key/opensfm
